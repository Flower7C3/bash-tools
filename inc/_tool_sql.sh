...
function symfony_database_migrate {
	...
	local symfonyConsole=$2
    local hasMigrations=$(${phpScript} ${applicationDir}${symfonyConsole} doc:mig:sta | grep "New Migrations" | sed 's/[^0-9]//g')

    if [[ "$hasMigrations" != "0" ]]; then
        printf "${NoticeB}Database has new structure${Color_Off} \n"
        ${phpScript} ${applicationDir}${symfonyConsole} doctrine:schema:update --dump-sql
        ${phpScript} ${applicationDir}${symfonyConsole} doc:mig:sta --show-versions | grep "not migrated"
        printf "${Black}${On_Cyan}${applicationDir}${symfonyConsole} doctrine:migrations:migrate${Color_Off}\n\n"
        ${applicationDir}${symfonyConsole} doctrine:migrations:migrate
    else
...
function mysql_remote_query_via_symfony {
	...
	ssh $hostName 'configFile='${applicationDir}'app/config/parameters.yml && sqlHost=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_host:\(.*\)/\2/p" | xargs` && sqlUser=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_user:\(.*\)/\2/p" | xargs` && sqlPass=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_password:\(.*\)/\2/p" | xargs` && sqlBase=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_name:\(.*\)/\2/p" | xargs` && printf "'$sqlQuery'" | mysql --host=$sqlHost --user=$sqlUser --password=$sqlPass $sqlBase'
}
...
function mysql_remote_export_via_symfony {
	...
	printf "${InfoB}Backup MySQL database at ${InfoBI}${hostMaster}${InfoB} host in ${InfoBI}${applicationDir}${InfoB} director (via Symfony config)y${Info}\n"
	ssh $hostName 'configFile='${applicationDir}'app/config/parameters.yml && sqlHost=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_host:\(.*\)/\2/p" | xargs` && sqlUser=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_user:\(.*\)/\2/p" | xargs` && sqlPass=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_password:\(.*\)/\2/p" | xargs` && sqlBase=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_name:\(.*\)/\2/p" | xargs` && mysqldump --routines --events --triggers --single-transaction --host=$sqlHost --user=$sqlUser --password=$sqlPass $sqlBase > ${HOME}/'${fileName}''
}
...
function mysql_remote_import_via_symfony {
	...
	printf "${InfoB}Import MySQL database at ${InfoBI}${hostName}${InfoB} host in ${InfoBI}${applicationDir}${InfoB} directory (via Symfony config)${Info}\n"
	ssh $hostName 'configFile='${applicationDir}'app/config/parameters.yml && sqlHost=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_host:\(.*\)/\2/p" | xargs` && sqlUser=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_user:\(.*\)/\2/p" | xargs` && sqlPass=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_password:\(.*\)/\2/p" | xargs` && sqlBase=`cat $configFile | sed -n "s/\([ ]\{1,\}\)database_name:\(.*\)/\2/p" | xargs` && mysql --host=$sqlHost --user=$sqlUser --password=$sqlPass $sqlBase < ${HOME}/'${fileName}''
}

function mysql_backup_via_symfony {
    local applicationDir=$1
    local symfonyConfigFilePath="${applicationDir}app/config/parameters.yml"
